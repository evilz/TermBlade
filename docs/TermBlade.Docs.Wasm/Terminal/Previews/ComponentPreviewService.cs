...
public static class ComponentPreviewService
{
  ...
  private static readonly HashSet<string> AdditionalPreviewComponents = new(StringComparer.OrdinalIgnoreCase)
  {
    "markup",
    "panel",
    "textpath",
  ...
  public static string RenderPreview(string component)
  {
    ...
    switch (component.ToLowerInvariant())
    {
      ...
      default:
        if (AdditionalPreviewComponents.Contains(component))
          RenderAdditionalComponent(buffer, component);
        else
          RenderUnknown(buffer, component);
        break;
    }
  ...
  private static void RenderSegmentedText(RenderBuffer b)
  {
    ...
    Draw(b, 2, 3, " BUILD ", Bg, TextAttributes.Bold, AccentFg);
    Draw(b, 10, 3, "TermBlade.Core ", DefaultFg);
    Draw(b, 25, 3, "passed", AccentFg, TextAttributes.Bold);
    Draw(b, 33, 3, "502 tests", BlueFg);
    Draw(b, 2, 5, " WARN ", Bg, TextAttributes.Bold, YellowFg);
    Draw(b, 9, 5, "preview height increased", YellowFg);
    Draw(b, 2, 7, " PATH ", Bg, TextAttributes.Bold, BlueFg);
    Draw(b, 9, 7, "docs/components.html", DefaultFg, TextAttributes.Underline);
    Draw(b, 2, 9, " ERROR ", Bg, TextAttributes.Bold, RedFg);
    Draw(b, 10, 9, "none", AccentFg, TextAttributes.Bold);
  }
  ...
  private static void RenderAdditionalComponent(RenderBuffer b, string component)
  {
    DrawBox(b, 2, 1, 48, 14, $" {component} ", CyanFg, "rounded");
    Draw(b, 5, 3, component, YellowFg, TextAttributes.Bold);
    Draw(b, 5, 5, "Dedicated Core renderable", AccentFg);
    Draw(b, 5, 6, "Dedicated Razor wrapper", BlueFg);
    Draw(b, 5, 7, "Dedicated gallery demo", MagentaFg);
    Draw(b, 5, 9, $"<{component} Width=\"40\" Height=\"8\" />", DefaultFg);
    Draw(b, 5, 11, $"{component}Demo.razor", MutedFg);
  }
